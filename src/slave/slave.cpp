...
void Slave::statusUpdate(StatusUpdate update, const UPID& pid)
{
  ...
  executor::Call::Update callUpdate;
  callUpdate.mutable_status()->CopyFrom(update.status());
  callUpdate.mutable_uuid()->CopyFrom(update.uuid());
  callUpdate.mutable_timestamp()->CopyFrom(update.timestamp());

  executor::Call call;
  call.mutable_update()->CopyFrom(update);
                         ...
                         const executor::Call& call)
{
  ...
  if (framework->state == Framework::TERMINATING) {
    LOG(WARNING) << "Ignoring status update " << call.update
  ...
  if (HookManager::hooksAvailable()) {
    ...
    TaskStatus statusFromHooks =
      HookManager::slaveTaskStatusDecorator(
          framework->id(), call.update.status());
  ...
  TaskStatus status = call.update.status();

  Executor* executor = framework->getExecutor(status.task_id());
  if (executor == NULL) {
    LOG(WARNING)  << "Could not find the executor for "
                  << "status update " << call.update;
    ...
    statusUpdateManager->update(call, info.id())
      .onAny(defer(self(), &Slave::__statusUpdate, lambda::_1, call));
  ...
  if (executor->pid.isSome() &&
      executor->pid.get() != UPID()) {
    LOG(WARNING) << "Received status update " << call.update
                 << " from " << executor->pid.get()
    ...
    bool checkpoint)
{
  if (future.isSome() && !future->isReady()) {
    ...
    Executor* executor = getExecutor(call.framework_id(), executorId);
    if (executor != NULL) {
      containerizer::Termination termination;
      termination.set_state(TASK_LOST);
      termination.add_reasons(TaskStatus::REASON_CONTAINER_UPDATE_FAILED);
  ...
  if (checkpoint) {
    ...
    statusUpdateManager->update(call, info.id(), executorId, containerId)
      .onAny(defer(self(), &Slave::__statuUpdate, lambda::_1, call));
  } else {
...
void Slave::__statusUpdate(
    ...
    const executor::Call& call)
{
  CHECK_READY(future) << "Failed to handle status update " << call.update;
  ...
  if (executor->pid != UPID()) {
    LOG(INFO) << "Sending acknowledgement for status update " << call.update
              << " to " << executor->pid;
    StatusUpdateAcknowledgementMessage message;
    message.mutable_framework_id()->MergeFrom(call.framework_id());
    message.mutable_slave_id()->MergeFrom(call.slave_id());
    message.mutable_task_id()->MergeFrom(call.update.status().task_id());
    message.set_uuid(call.update.uuid());
...
void Slave::forward(executor::Call call)
{
  CHECK(state == RECOVERING || state == DISCONNECTED ||
        state == RUNNING || state == TERMINATING)
    << state;
  ...
  if (state != RUNNING) {
    LOG(WARNING) << "Dropping status update " << call.update
  ...
  if (framework != NULL) {
    StatusUpdate update;
    update.mutable_status()->CopyFrom(call.update.status());
    update.mutable_uuid()->CopyFrom(call.update.uuid());
    update.mutable_timestamp()->CopyFrom(call.update.timestamp());
    update.mutable_framework_id()->CopyFrom(call.framework_id());
    update.mutable_executor_id()->CopyFrom(call.executor_id());
    update.mutable_slave_id()->CopyFrom(call.update.status.slave_id());
